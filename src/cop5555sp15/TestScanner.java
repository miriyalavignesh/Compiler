...
public class TestScanner {
	...
	@Test
	public void emptyInput1() {
		System.out.println("Test: emptyInput1");
		...
		assertEquals(ILLEGAL_CHAR, stream.nextToken().kind);// Java default char \0
		assertEquals(EOF, stream.nextToken().kind);
	...
	public void separators() {
		...
		assertEquals(COLON, stream.nextToken().kind);
		assertEquals(QUESTION, stream.nextToken().kind);
		assertEquals(EOF, stream.nextToken().kind);
		assertEquals(3, stream.tokens.get(11).getLineNumber());// 5th token is on
	...
	public void operator() {
		...
		assertEquals(EQUAL, stream.nextToken().kind);
		assertEquals(ASSIGN, stream.nextToken().kind);
		assertEquals(BAR, stream.nextToken().kind);
		assertEquals(AND, stream.nextToken().kind);
		assertEquals(NOTEQUAL, stream.nextToken().kind);
		assertEquals(PLUS, stream.nextToken().kind);
		assertEquals(MINUS, stream.nextToken().kind);
		assertEquals(TIMES, stream.nextToken().kind);
	...
	@Test
	public void stringLiteralsWithWhiteSpace() {
		System.out.println("stringLiteralsWithWhiteSpace");
		String input = " \"abc\nws\" \"def\rws\" \"ghijk\" \"123\" \"&^%$\" ";
		System.out.println(input);
		TokenStream stream = new TokenStream(input);
		Scanner scanner = new Scanner(stream);
		scanner.scan();
	...
	@Test
	public void stringLiteralsQuotesInside() {
		System.out.println("stringLiteralsQuotesInside");
		char [] input = {' ','"','b','c','\\','"','d','"'};
		System.out.println(input);
		TokenStream stream = new TokenStream(input);
		Scanner scanner = new Scanner(stream);
		...
		char [] output = {'b','c','"','d',};
		String[] expectedTexts = { String.valueOf(output,0,output.length), "" }; 
		assertArrayEquals(expectedKinds, makeKindArray(stream));
		assertArrayEquals(expectedTexts, makeTokenTextArray(stream));
	}
		//string literals use \n \r \"and check linenumbers
	@Test
